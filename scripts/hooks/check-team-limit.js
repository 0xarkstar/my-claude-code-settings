 ...
 * Prevents tmux race conditions (Issue #23615).
 *
...
process.stdin.on("end", () => {
  try {
    ...
    // Not a team spawn — regular subagent, allow
    ...
    // Team doesn't exist yet — first spawn, allow
    if (!fs.existsSync(configPath)) process.exit(0);

    const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
    // Count only active non-lead members (team-lead doesn't count toward limit)
    ...
    if (activeCount >= 3) {
      ...
      console.error(
        "[Hook] Wait for existing teammates to complete their tasks and shut down naturally"
      );
      console.error(
        "[Hook] Then spawn new teammates. Do NOT force-shutdown active workers"
      );
      process.exit(2);
    }
  } catch (e) {
